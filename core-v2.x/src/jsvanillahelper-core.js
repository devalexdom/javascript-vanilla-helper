...
export class JSVanillaHelper {
    constructor(target = null, targetData = {}, helperData = { reg: { mainAppRef: null, appsRef: {}, workers: {}, pNTouchGesturesHelperFunc: null }, flags: {} }) {
        this.version = 2.03;
    ...
    scrollContainerToTarget({ yOffset = 0, xOffset = 0, behavior = 'smooth' }, parentEl = null, t = this.t) {
    ...
    isEmpty(t = this.t) {
        if (t == null)
            return true;
        const type = typeof t;
        if (type === 'string' || Array.isArray(t))
            return this.isZeroLength(t);
    ...
    mutationObserver(actionCallback, { observeAttributes = ["class"], observeMutationTypes = [], observeMultipleMutations = false, nativeObserverConfig = { attributes: true } } = {}, t = this.t) {
        const stopObserver = () => {
            observer.disconnect();
        };
    ...
    isElementBound(child, padding, t = this.t) {
        return (t.getBoundingClientRect().right >=
            child.getBoundingClientRect().right + padding); // helper t as parent
    }
    getVisibilityData(verticalOffset = 0, t = this.t) {
        ...
        if (!(cR.height == 0 && cR.width == 0 && cR.top == 0 && cR.left == 0)) {
            rendered = true;
            const vertInView = cR.top - verticalOffset <= windowHeight &&
    ...
    onScroll({ offsetTop = 0, top: topCallback, down: downCallback, up: upCallback, disableFlagMode = false, }, t = this.t) {
        let downFlag = false;
        let lastScroll = 0;
        ...
        t.addEventListener('scroll', () => {
            ...
            if (!disableFlagMode) {
                if (currentScroll > lastScroll && !downFlag) {
                    ...
                    downFlag = true;
                    if (downCallback)
                        downCallback(setOffsetTop);
                }
                else if (currentScroll < lastScroll && downFlag) {
                    // up
                    downFlag = false;
                    if (upCallback)
                        upCallback(setOffsetTop);
                }
            }
    ...
    AJAX({ url = '', requestMethod = 'POST', withCredentials = false, dataToSend = null, async = true, enableJson = { onResponse: false, onSend: false, onError: false }, requestContent = 'text', timeoutError = 10000, }, actions = {
        onSuccess: (callback) => { },
        onError: (callback) => { },
...
//# sourceMappingURL=jsvanillahelper-core.js.map