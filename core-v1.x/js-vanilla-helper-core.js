class JSVanillaHelper {
  constructor(
    target = null,
    targetData = {},
    helperData = { reg: { workers: {} }, flags: {} }
  ) {
    this.versionNumber = 1.96;
  ...
  backwardCompatibility() {
    this.fastEach = this.reverseEach;
  }
  ...
  alterFontSize(pixelsInInt = -2, t = this.t) {
    const fontSize = parseInt(
      window.getComputedStyle(t).fontSize.replace('px', '')
    );
    t.style.fontSize = `${fontSize + pixelsInInt}px`;
  }

  setMaxViewportScale(maximumScaleStr = '', initialScaleStr = '1.0') {
    const maxScale = maximumScaleStr
      ? `, maximum-scale=${maximumScaleStr}`
      : '';
    const userScalable =
      maximumScaleStr === initialScaleStr ? ', user-scalable=no' : '';
    const vMetaContent = `width=device-width, initial-scale=${initialScaleStr}${maxScale}${userScalable}`;
    const viewportMeta = document.querySelector('meta[name=viewport]');
    if (viewportMeta) {
      viewportMeta.content = vMetaContent;
    } else {
      this.addMeta('viewport', vMetaContent, document.head);
    }
    if (maximumScaleStr === initialScaleStr) {
  ...
  isEmpty(t = this.t) {
    ...
    const type = typeof t;
    if (type === 'string' || type === 'array') return this.isZeroLength(t);
  ...
  onEvent(eventName, actionCallback, t = this.t) {
    ...
    return { helper: this, removeListener: removeListener };
  }
  ...
  addHelperExtension(extensionsObj) {
    const extensionName = extensionsObj.helperExtensionName;
  ...
  getTextRenderedSize(font = '16px Arial', widthLimit = 0, t = this.t) {
    ...
    return { width: textMesure.width, height: height, maxFontSizePX };
  }
  ...
  mutationObserver(actionCallback, { observeAttributes = ["class"], observeMutationTypes = [],
    observeMultipleMutations = false, nativeObserverConfig = { attributes: true } } = {}, t = this.t) {
    const stopObserver = () => {
      observer.disconnect();
    };
    ...
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        if (observeAttributes.indexOf(mutation.attributeName) > -1 || observeMutationTypes.indexOf(mutation.type) > -1) {
          callback(mutation);
  ...
  AJAX(
    ...
    xhr.onload = () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        handleResponse(enableJson.onResponse, ajaxHelperObj);
        actions.onSuccess(ajaxHelperObj);
      } else if (xhr.status >= 400 && xhr.status < 600) {
        handleResponse(enableJson.onError, ajaxHelperObj);
...
const V$C = (cN = '') => {
  return v.setTarget(document.getElementsByClassName(cN));
};

const V$I = (id = '') => {
  return v.setTarget(document.getElementById(id));
};

const V$ = (t = null) => {
  return v.setTarget(document.querySelectorAll(t));
};
